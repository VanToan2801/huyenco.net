...
const InputPage = ({ onAnalyze, loading, automationData }) => {
  ...
  return <BirthInput onAnalyze={handleAnalyze} loading={loading} automationData={automationData} />;
};
...
const AppContent = () => {
  ...
  const navigate = useNavigate();
  const { liveData, clearLiveData } = useLiveStream();
  const [automationData, setAutomationData] = React.useState(null);

  // Kịch bản tự động hoá Live Stream Tiktok
  useEffect(() => {
    if (liveData) {
      console.log("[App Automation] Bắt đầu chuỗi tự động hoá", liveData);
      
      // B1: Về trang chủ
      navigate('/');
      
      // Cấu hình params lá số
      const baziParams = {
        name: liveData.name || 'Khách TikTok',
        gender: liveData.gender || 'Nữ',
        calendar: 'solar',
        day: liveData.day,
        month: liveData.month,
        year: liveData.year,
        hour: liveData.hour ?? 12,
        minute: 0
      };

      // Gửi dữ liệu này xuống BirthInput để giao diện tự render
      setAutomationData(baziParams);

      // B2: Đợi 3s để khán giả nhìn thấy form đã được điền, rồi mới nhảy qua lá số
      setTimeout(() => {
          analyze(baziParams).then(res => {
            if (res) {
               // Đang ở /laso, chờ khán giả xem lá số rồi tự nhảy qua Tư Vấn
               console.log("[App Automation] Đã xem lá số, chuẩn bị nhảy sang tab Tư Vấn...");
               setTimeout(() => {
                   navigate('/tuvan', {
                       state: {
                           autoTrigger: true,
                           autoQuestion: liveData.question,
                           autoAnswer: liveData.answer
                       }
                   });
               }, 6000); // Tăng thời gian đứng ở tab lá số lên 6 giây cho chắc !!
            }
          });
          
          // Xoá memory để tránh render lại
          setAutomationData(null);
      }, 3000); 

      clearLiveData(); // Xoá data event để không lặp lại
    }
  }, [liveData, navigate, analyze, clearLiveData]);
  ...
  return (
    <HelmetProvider>
      <Shell hasData={hasData} onClearData={clearData}>
        ...
        <Routes>
          {/* Input form routes */}
          <Route path="/" element={<InputPage onAnalyze={analyze} loading={loading} automationData={automationData} />} />
