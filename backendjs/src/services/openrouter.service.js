...
class OpenRouterService {
     ...
     */
    async generateAnswer(baziContext, luckCyclesData, questionText, personaId = 'huyen_co', partnerContext = null, userName = null) {
        if (!this.apiKey) {
            throw new Error('OPENROUTER_API_KEY is not configured');
        }

        const systemPrompt = this.buildSystemPrompt(personaId);
        const userPrompt = this.buildUserPrompt(baziContext, luckCyclesData, questionText, personaId, partnerContext, userName);

        console.log(`[OpenRouter] [Question]: "${questionText}"`);
        // console.log(`[OpenRouter] [Full User Prompt]:`, userPrompt);
...
2. Phân tích 2-3 điểm CHÍNH, mỗi điểm 1-2 câu. Phải cực kỳ ngắn gọn, súc tích, PHẢI TẬP TRUNG TRẢ LỜI ĐÚNG CÂU HỎI THỰC TẾ của người dùng, không được trả lời lan man ngoài lề lá số mà bỏ quên câu hỏi.
3. Nếu câu hỏi không liên quan đến lá số (ví dụ hỏi thời tiết), hãy khéo léo từ chối và hướng sự tập trung về mệnh cách.
4. KHÔNG dùng cụm từ "AI", "máy móc".
5. Ở cuối cùng, luôn cung cấp một phần có tiêu đề [FOLLOW_UP] chứa 3-5 câu hỏi gợi mở dựa trên lá số và đại vận của người dùng.
6. Mỗi câu hỏi gợi mở phải là một dòng bắt đầu bằng dấu "-". Những câu hỏi này phải thực sự liên quan đến rủi ro hoặc cơ hội sắp tới của chủ mệnh, trong đấy có 1 câu liên quan đến ngày, tháng sắp tới.`;
    }

    /**
     * Build user prompt with BaZi context
     */
    buildUserPrompt(baziContext, luckCyclesData, questionText, personaId, partnerContext = null, userName = null) {
...
**Thông tin cơ bản:**
- Tên: ${userName || basicInfo.ten || 'Mệnh chủ'}
- Giới tính: ${basicInfo.gioi_tinh || 'Nam'}
- Ngày sinh dương lịch: ${basicInfo.ngay_duong_lich || 'N/A'}
- Ngày sinh âm lịch: ${basicInfo.ngay_am_lich || 'N/A'}
- Giờ sinh: ${basicInfo.gio_sinh || 'N/A'}
- Mệnh (Ngũ Hành Nạp Âm): ${basicInfo.menh || baziContext.chi_tiet_tru?.[0]?.nap_am || 'N/A'}
...
## CÂU HỎI CỦA NGƯỜI DÙNG (CỰC KỲ QUAN TRỌNG):
 
 >>> "${questionText}" <<<
 
 ---
 
 Hãy tập trung toàn bộ sự chú ý để phân tích và trả lời DUY NHẤT câu hỏi trên dựa trên lá số Bát Tự được cung cấp. Không được trả lời chung chung về lá số mà không giải quyết vấn đề trong dấu ngoặc kép.
 
 YÊU CẦU QUAN TRỌNG:
 1. Trả lời bằng phong cách của nhân vật ${personaId === 'menh_meo' ? 'Thầy Mệnh Mèo GenZ' : 'Thầy Huyền Cơ Bát Tự'}.
 2. Đưa ra 2-3 đoạn văn cực kỳ ngắn gọn, súc tích, ĐÁP ỨNG TRỰC TIẾP CÂU HỎI CỦA MỆNH CHỦ.
